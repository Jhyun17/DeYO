...
class SAR(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, steps=1, episodic=False, margin_e0=0.4*math.log(1000), reset_constant_em=0.005):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs, ema, reset_flag = forward_and_adapt_sar(x, self.model,
                                                             self.optimizer,
                                                             self.margin_e0,
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    if ema is not None:
        if ema < reset_constant:
            print("ema < {}, now reset the model".format(reset_constant))
            reset_flag = True
